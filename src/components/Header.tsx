...
import { Menu, Heart, Scale, X, type LucideIcon, Info, LayoutGrid, Moon, Sun, Globe } from "lucide-react";
    ...
    >
      ...
      {count !== undefined && count > 0 && (
        <span className={cn(
            ...
            activePage === page ? "border-white text-white" : "border-black text-black"
        )}>
          {count}
        </span>
      )}
    ...
    <header className="sticky top-0 z-[60] w-full border-b border-black bg-white">
      <div className="flex h-16 items-stretch justify-between">
        ...
        >
            <div className="flex items-center">
                <svg width="110" height="22" viewBox="0 0 215 42" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <nav className="hidden md:flex items-stretch h-full ml-auto">
          ...
          <NavItem page="home" label={t('nav.catalog')} icon={LayoutGrid} />
          <NavItem page="compare" label={t('nav.compare')} icon={Scale} count={compareCount} />
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-16 z-50 bg-white flex flex-col overflow-y-auto">
          ...
          >
             <span className="text-4xl font-bold tracking-tighter uppercase">{t('nav.catalog')}</span>
          ...
          >
             <div className="flex items-baseline gap-4">
                <span className="text-4xl font-bold tracking-tighter uppercase">{t('nav.compare')}</span>
                {compareCount > 0 && <span className="font-mono text-xl">({compareCount})</span>}
             </div>
             <Scale className="w-8 h-8 group-hover:scale-110 transition-transform" />
          </button>

          <button 
             onClick={() => { setActivePage("favorites"); setIsMobileMenuOpen(false); }}
             className="flex items-center justify-between p-8 border-b border-black hover:bg-black hover:text-white transition-colors group"
          >
             <div className="flex items-baseline gap-4">
                <span className="text-4xl font-bold tracking-tighter uppercase">{t('nav.favorites')}</span>
                {favoritesCount > 0 && <span className="font-mono text-xl">({favoritesCount})</span>}
             </div>
          ...
          <button 
             ...
             className="flex items-center justify-between p-8 border-b border-black hover:bg-black hover:text-white transition-colors group"
          >
             <span className="text-4xl font-bold tracking-tighter uppercase">{t('nav.about')}</span>
