...
export const FontCard: React.FC<FontCardProps> = ({
  ...
  const { t } = useLanguage();
  const [localWeight, setLocalWeight] = useState(400);
      ...
      <div className="group border-b border-black bg-white min-h-[300px] flex flex-col md:flex-row transition-colors hover:bg-neutral-50">
        ...
        <div className="w-full md:w-72 flex-shrink-0 p-4 md:p-6 border-r border-black flex flex-col justify-between relative">
           <div>
              <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-1">
                      {font.languages.includes("Cyrillic") && <span className="font-mono text-[9px] uppercase border border-black px-1 pt-0.5">CYR</span>}
                      {font.variable && <span className="font-mono text-[9px] uppercase bg-black text-white border border-black px-1 pt-0.5">VAR</span>}
                  </div>
              </div>
              ...
              {/* Technical Controls (Variable only here, size is global) */}
              <div className="space-y-6 opacity-20 group-hover:opacity-100 transition-opacity duration-300">
                  {font.variable && (
                      <div className="space-y-2">
                          ...
                          <input 
                              type="range" min="100" max="900" step="100" value={localWeight} onChange={(e) => setLocalWeight(Number(e.target.value))}
           ...
           <div className="flex gap-4 pt-6 border-t border-black/10 mt-auto">
               <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(font.id); }} className="hover:opacity-50 transition-opacity">
                   <Heart className={cn("w-4 h-4", isFavorite ? "fill-black" : "stroke-black")} />
               </button>
               <button onClick={(e) => { e.stopPropagation(); onToggleCompare(font.id); }} className="hover:opacity-50 transition-opacity flex items-center gap-1 group/btn">
                   {isCompared ? (
                        <div className="bg-black text-white p-0.5"><Check className="w-3 h-3" /></div>
                   ) : (
               ...
               <button onClick={(e) => { e.stopPropagation(); window.open(font.sourceUrl, '_blank'); }} className="hover:opacity-50 transition-opacity ml-auto">
        ...
        >
            ...
            <div className="flex-grow flex items-center justify-center overflow-hidden">
                <p className="text-black leading-tight text-center break-words w-full transition-all duration-200" style={style}>
                    {displayPreview}
                </p>
            </div>
        </div>
  ...
  return (
    <div className="group bg-white border-r border-b border-black flex flex-col h-[350px] relative hover:bg-neutral-50 transition-colors">
       ...
       <div className="p-3 flex justify-between items-start border-b border-black/10">
           <div>
               <h3 className="font-bold text-sm text-black tracking-tight">{font.name}</h3>
               <p className="font-mono text-[9px] text-neutral-500 uppercase">{font.categories[0]}</p>
           </div>
           <button onClick={() => onToggleFavorite(font.id)}>
               <Heart className={cn("w-3 h-3", isFavorite ? "fill-black" : "stroke-black")} />
           </button>
       </div>
       ...
       <div 
           ...
           onClick={() => onViewDetails(font.id)}
       >
           <p className="text-center leading-tight transition-all duration-200" style={{ ...style, fontSize: `${Math.min(fontSize, 60)}px` }}>
               {displayPreview}
           </p>
       </div>

       {/* Footer Stats - Unified */}
       <div className="h-10 px-4 border-t border-black/10 flex justify-between items-center">
            <button 
                onClick={(e) => { e.stopPropagation(); onToggleCompare(font.id); }} 
