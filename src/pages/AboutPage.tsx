...
import { ArrowUpRight, Scan, Grid, Layers, Box, Square, Send, Plus } from "lucide-react";
import { useLanguage } from "@/lib/i18n";
import { Link } from "react-router-dom";
import sergeiPhoto from 'figma:asset/aa411255aff9100a5119b6cb2cadfd4bdcbb57fd.png';
import MarkBuilder from "@/components/figma/MarkBuilder";
...
// --- MOUSE COORDINATES COMPONENT ---
const SystemCoordinates = ({ mouseX, mouseY }: { mouseX: any, mouseY: any }) => {
    const [coords, setCoords] = useState({ x: 0, y: 0 });
    useEffect(() => {
        const update = () => setCoords({ x: Math.round(mouseX.get()), y: Math.round(mouseY.get()) });
...
// --- HERO KINETIC LETTER ---
const HeroLetter = ({ char, mouseX, mouseY }: { char: string, mouseX: any, mouseY: any }) => {
    const ref = useRef<HTMLDivElement>(null);
    const [weight, setWeight] = useState(800); // Default heavy
    ...
    useEffect(() => {
        const update = () => {
            ...
            const radius = 800; // Large radius for hero

            if (dist < radius) {
                const factor = 1 - dist / radius; // 0 to 1
                // Proximity -> Lighter weight (Deconstruct)
                // Base 800 -> Drops to 100 near mouse
...
// --- SYSTEM KINETIC LETTER (For the "SYSTEM" block) ---
const KineticLetter = ({ char, mouseX, mouseY }: { char: string, mouseX: any, mouseY: any }) => {
    const ref = useRef<HTMLDivElement>(null);
    ...
    React.useEffect(() => {
        const update = () => {
            ...
            if (dist < radius) {
                ...
                setSlant((mouseX.get() - centerX) * 0.05);
            } else {
                setWeight(100);
                setSlant(0);
            }
...
// --- MARK BUILDER REMOVED (Moved to separate component) ---
...
export const AboutPage: React.FC<AboutPageProps> = ({ onNavigateHome }) => {
  ...
  return (
    <div ref={containerRef} onMouseMove={handleMouseMove} className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white overflow-x-hidden relative">
      <Noise />
      ...
      <section className="relative min-h-[80vh] md:min-h-screen flex flex-col border-b border-black overflow-hidden">
          ...
          <div className="absolute top-24 right-6 z-40 pointer-events-none hidden md:block">
            ...
            <div className="mt-1 opacity-50 font-mono text-[10px] text-neutral-400 text-right">SYSTEM_ACTIVE</div>
          </div>
             ...
             <div className="w-full mt-12 md:mt-24 relative">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                    <p className="font-mono text-[10px] uppercase tracking-widest text-neutral-500 max-w-xs leading-relaxed">
                        ИНДУСТРИАЛЬНЫЙ ИНДЕКС ОТКРЫТОЙ<br/>
                        ТИПОГРАФИКИ.
                    </p>
               ...
               <div className="p-6 md:p-12 flex flex-col justify-between order-2 lg:order-1 border-t lg:border-t-0 lg:border-r border-white/20 bg-black min-h-[50vh] lg:min-h-0">
                    <div>
                        ...
                        <h3 className="text-4xl md:text-7xl font-bold tracking-tighter mb-8 leading-none">
                            {t('landing.agency.headline_1')}<br/>
                            <span className="text-neutral-500">{t('landing.agency.headline_connect')}</span><br/>
                    ...
                    <div className="flex flex-col gap-6 mt-12">
                         <a href="https://imon.agency" target="_blank" rel="noreferrer" className="group flex items-center justify-between border-b border-white/20 pb-4 hover:border-white transition-colors">
                             <span className="font-mono text-xs uppercase tracking-widest">{t('landing.agency.link_agency')}</span>
                         ...
                         <a href="https://t.me/imonsergei" target="_blank" rel="noreferrer" className="group flex items-center justify-between border-b border-white/20 pb-4 hover:border-white transition-colors">
                             <span className="font-mono text-xs uppercase tracking-widest">{t('landing.agency.link_contact')}</span>
                             <div className="flex items-center gap-2 font-bold text-lg md:text-xl group-hover:translate-x-2 transition-transform">
               ...
               <div className="relative order-1 lg:order-2 h-[50vh] lg:h-auto overflow-hidden group cursor-none">
                    ...
                    <div className="absolute inset-0 flex flex-col justify-between p-6 md:p-12 mix-blend-difference pointer-events-none z-20">
                         <div className="flex justify-between items-start">
                              ...
                              <span className="font-mono text-xs text-white uppercase tracking-widest">Fig. 03</span>
                         </div>
      ...
      <section className="border-b border-black bg-white">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 w-full">
              {[
                  ...
                  <div key={i} className={`relative p-6 md:p-12 border-b sm:border-b-0 ${i !== 3 ? 'sm:border-r border-black' : ''} group hover:bg-black hover:text-white transition-colors duration-300 flex flex-col min-h-[200px] md:h-full`}>
      ...
      <div className="bg-black text-white py-4 overflow-hidden border-b border-black">
          ...
          >
              {Array(10).fill("Typography is not just reading. It is seeing.").map((text, i) => (
      ...
      <div className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center font-mono text-[10px] uppercase tracking-widest text-neutral-500 bg-white gap-4">
           ...
           <div className="flex gap-6">
               <Link to="/privacy" className="hover:text-black cursor-pointer transition-colors">Privacy Policy</Link>
               <Link to="/terms" className="hover:text-black cursor-pointer transition-colors">Terms of Use</Link>
               <Link to="/license" className="hover:text-black cursor-pointer transition-colors">License</Link>
           </div>
      </div>
    </div>
  );
